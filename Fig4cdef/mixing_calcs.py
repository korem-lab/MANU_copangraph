...
from evaluate_assembly import get_coverage_metrics, align_nodes, n50, num_edges, num_nodes, get_top_alignments
from AssemblyParser import Assembly
from collections import defaultdict
import re

COPANGRAPHS = '../data/Fig4cdef/gherig/copangraphs/*.gfa'
MEGAHIT_GRAPHS = '../data/Fig4cdef/MEGAHIT_graphs/*.fastg'

GENOMES = '../data/Fig3efgh/coassembly_reference_metagenomes/9_sample_reference.fasta'
...
def parse_name(s, gherig=True):
    if gherig:
        if 'fastg' in s:
            ...
            return 'copangraph', float(re.findall('sd_(0.[0-9]+)',s)[0])
    else:
        if 'fastg' in s:
            fields = s.split('/')
...
if __name__ == '__main__':
    ...
    #graph, window_sz = sys.argv[1], sys.argv[2]
    #window_sz = int(window_sz)
    # default the window size to 10
    graph = sys.argv[1]
    window_sz = 10
    ...
    cov_record = get_coverage_metrics(
        name, alignments, genomes, genome_lens, asm, g_to_aln_idx, unaligned_contigs, contig_map, disregard_unmapped=True
    )
    ...
    nodes = num_nodes(asm)
    edges = num_edges(asm)
    tool, param = parse_name(graph) 
    is_copan  = True if 'fastg' not in graph else False
    cov_records.append([tool, param, name, n_50, nodes, edges, 'cov_tp', cov_record.cov_tp])
    cov_records.append([tool, param, name, n_50, nodes, edges, 'cov_fn', cov_record.cov_fn])
    cov_records.append([tool, param, name, n_50, nodes, edges, 'cov_fp', cov_record.cov_fp])
    cov_records.append([tool, param, name, n_50, nodes, edges, 'cov_precsion', prc])
    cov_records.append([tool, param, name, n_50, nodes, edges, 'cov_recall', rcl])
    cov_records.append([tool, param, name, n_50, nodes, edges, 'cov_F-score', fsc])
    cov_records = pd.DataFrame(cov_records, columns=['assembler', 'parameter', 'name', 'n50', 'num_nodes', 'num_edges', 'metric', 'value'])
    ...
    multi_genome_status = aln_df.groupby('contig_name').apply(
        lambda x: pd.Series(
            [tool, param, name, n_50, nodes, edges, 
             x.genome.nunique() > 1, x.shape[0], multigenome_coverage_bp, sum(genome_lens), sum_maxlen_bp], 
            index=['assembler', 'parameter', 'name', 'n50', 'num_nodes', 'num_edges', 
    ...
    # merge to get the max len for each mode
    multi_genome_status = pd.merge(multi_genome_status, max_len, on='node_id', how='inner')

    mgs_record = pd.DataFrame([[
        tool, param, name, n_50, nodes, edges, sum_maxlen_bp, sum(genome_lens),
        ...
        'assembler', 'param', 'name', 'n50', 'num_nodes', 'num_edges', 'sum_maxlen_bp', 'total_ref_bps',
    ...
    cov_records.to_csv(os.path.join(OUTDIR, f'{tool}_{param}_coverage_records.csv'), index=False)
    mgs_record.to_csv(os.path.join(OUTDIR, f'{tool}_{param}_multigenome_record.csv'), index=False)
