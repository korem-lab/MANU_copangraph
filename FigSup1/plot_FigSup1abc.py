...
def run_adonis(X_fl, outcome_fl, is_factor, is_dist, X_nm = '../data/FigSup1/adonis/X_', y_nm='../data/FigSup1/adonis/y_'):
    os.makedirs('../data/FigSup1/adonis',exist_ok=True)
    _uuid=str(uuid4())
    X_nm+=_uuid +'.csv'
    X_fl.to_csv(X_nm)
    y_nm+=_uuid +'.csv'
...
if __name__ == '__main__':
    ...
    labels = pd.read_csv('../data/FigSup1/Labels.csv')
    labels.index = labels.StoolID
    df = read_copan('../data/FigSup1/graph_euclid.abnd_mat.pkl',labels.index.astype(str))
    df=occ_filter(df,10,40,0,'.abnd_mat.pkl')
    ...
    ax.set_title(f'vrecol e r:{adonis.R2[0]:.4f},F:{adonis.F[0]:.4f},p:{adonis["Pr(>F)"][0]},N:{df.shape[1]}')
    ax.xaxis.set_major_locator(MaxNLocator(nbins=5))
    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
    name='../data/FigSup1/FigSup1c'
    ...
    df = read_copan('../data/FigSup1/graph_jaccard.abnd_mat.pkl',labels.index.astype(str))
    ...
    plt.figure(figsize=(2.5,2.5))
    ax = sns.scatterplot(data=coords, x='PC1', y='PC2', hue='outcome',alpha=0.4,s=50)
    ax.set_title(f'vrecol j r:{adonis.R2[0]:.4f},F:{adonis.F[0]:.4f},p:{adonis["Pr(>F)"][0]},N:{df.shape[1]}')
    ax.xaxis.set_major_locator(MaxNLocator(nbins=5))
    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
    name='../data/FigSup1/FigSup1b'
    plt.savefig(name+'_LBL.pdf',dpi=1400, bbox_inches='tight')
    plt.savefig(name+'_LBL.png',dpi=900, bbox_inches='tight')
    plot_unlabelled_version(ax,name)

    df = pd.read_pickle('../data/FigSup1/mOTUs.df')
    df=df.loc[labels.index.astype(str),:]
    ...
    plt.figure(figsize=(2.5,2.5))
    ax = sns.scatterplot(data=coords, x='PC1', y='PC2', hue='outcome',alpha=0.4,s=50)
    ax.set_title(f'vrecol bc r:{adonis.R2[0]:.4f},F:{adonis.F[0]:.4f},p:{adonis["Pr(>F)"][0]},N:{df.shape[1]}')
    ax.xaxis.set_major_locator(MaxNLocator(nbins=5))
    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
    name='../data/FigSup1/FigSup1a'
    plt.savefig(name+'_LBL.pdf',dpi=1400, bbox_inches='tight')
    plt.savefig(name+'_LBL.png',dpi=900, bbox_inches='tight')
    plot_unlabelled_version(ax,name)
