...
if __name__ == '__main__':
    ...
    labels = pd.read_csv('../data/Fig6abcdef/crc/Labels.csv', index_col=0)
    df = read_copan('../data/Fig6abcdef/crc/graph_euclid.abnd_mat.pkl', labels.index.astype(str))
    df=occ_filter(df,20,33,100,'.abnd_mat.pkl')
    ...
    ax.set_title(f'crc e r:{adonis.R2[0]:.4f},F:{adonis.F[0]:.4f},p:{adonis["Pr(>F)"][0]},N:{df.shape[1]}')
    ax.xaxis.set_major_locator(MaxNLocator(nbins=5))
    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
    name='../data/Fig6abcdef/Fig6c'
    ...
    df = read_copan('../data/Fig6abcdef/crc/graph_jaccard.abnd_mat.pkl', labels.index.astype(str))
    df=occ_filter(df,10,40,250,'.abnd_mat.pkl')
    ...
    plt.figure(figsize=(2.5,2.5))
    ax = sns.scatterplot(data=coords, x='PC1', y='PC2', hue='outcome',alpha=0.4,s=50)
    ax.set_title(f'crc j r:{adonis.R2[0]:.4f},F:{adonis.F[0]:.4f},p:{adonis["Pr(>F)"][0]},N:{df.shape[1]}')
    ax.xaxis.set_major_locator(MaxNLocator(nbins=5))
    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
    name='../data/Fig6abcdef/Fig6b'
    plt.savefig(name+'_LBL.pdf',dpi=1400, bbox_inches='tight')
    plt.savefig(name+'_LBL.png',dpi=900, bbox_inches='tight')
    plot_unlabelled_version(ax,name)

    df = pd.read_pickle('../data/Fig6abcdef/crc/mOTUs.df')
    ci=df.index.intersection(labels.index)
    df=df.loc[ci,:]
    _labels=labels.loc[ci,:]
    D=get_dist(df,'braycurtis')
    adonis=run_adonis(pd.DataFrame(D, index=_labels.index, columns=_labels.index), _labels, True, True) 
    ordination= pcoa(D)
    coords= ordination.samples
    coords['outcome'] = _labels.outcome.values
    print('PC1 - ', ordination.proportion_explained['PC1'], 'PC2 - ', ordination.proportion_explained['PC2'])
    plt.figure(figsize=(2.5,2.5))
    ax = sns.scatterplot(data=coords, x='PC1', y='PC2', hue='outcome',alpha=0.4,s=50)
    ax.set_title(f'crc bc r:{adonis.R2[0]:.4f},F:{adonis.F[0]:.4f},p:{adonis["Pr(>F)"][0]},N:{df.shape[1]}')
    ax.xaxis.set_major_locator(MaxNLocator(nbins=5))
    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
    name='../data/Fig6abcdef/Fig6a'
    plt.savefig(name+'_LBL.pdf',dpi=1400, bbox_inches='tight')
    plt.savefig(name+'_LBL.png',dpi=900, bbox_inches='tight')
    plot_unlabelled_version(ax,name)
