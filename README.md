...
| `Fig1/raw_graph.gfa.gz` | Copangraph of the gut metagenomic samples. |
| `Fig1/raw_graph.ncolor.csv.gz` | csv of the node x sample occurrence matrix output by copangraph. |
| `Fig1/sample_map.csv` | List of samples in the copangraph graph. |
| `Fig1/persistence_table.csv` | Whether the sample has a persistence or clearance label. |
| `Fig1/metabat2bins.tar` | MetaBAT2 bins in format `<sample_name>.bin.fasta.gz` |
| `Fig1/binned_nodes.csv.gz` | csv produced by `nodes_of_interest.py` listing whether each node contains intervals from binned or only unbinned contigs. |
| `Fig1/reduced.gfa.gz` | Reduced gfa produced by `nodes_of_interest.py`. |
| `Fig1/gtdbtk.bac120.summary.tsv` | taxonomic classification of each contig by GTDB-Tk. |
| `Fig1/Figure2.gephi` | Gephi file specifying the layout of the final graph visualized in Figure 1. |
|--|--|
...
| `Fig3bd/AssemblyParser.py, Fig3bd/parse_seq.py, Fig3bd/evaluate_assembly.py/` | Auxiliary files used by `run_simulated_eval.py`. |
...
| `Fig3efgh/AssemblyParser.py, Fig3efgh/parse_seq.py, Fig3efhg/evaluate_assembly.py` | Auxiliary files used by `run_real_eval.py`. |
