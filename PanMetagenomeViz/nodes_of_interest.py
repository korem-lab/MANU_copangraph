...
def extract_relevant_classifications(gtdbtk):
...
def filter_gfa(gfa, noi, min_samples=0, min_degree=0, always_include_interesting=True):
    ...
    with open(gfa) as f:
        for e in ps.parse_gfa(f):
            if e.type == ps.GFATypes.S:
                ...
                links.append(e)
    node_index = {str(e):i for i, e in enumerate(set(noi.node))}
    ...
    for idx in tqdm.tqdm(set(noi.index)):
        ...
        is_interesting = noi.loc[idx, 'is_interesting']
        if is_interesting and always_include_interesting:
...
if __name__ == '__main__':
    ...
    #    filter_gfa(gfa, noi, min_samples=4, min_degree=1, always_include_interesting=True)
    ...
    #relevant_bins = extract_relevant_classifications(gtdbtk)
    ...
    filter_gfa(gfa, noi, min_samples=8, min_degree=2, always_include_interesting=True)
