...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: <exe> <cpg.gfa> <cpg_edge_table> <sample_list>')
        sys.exit()
    ...
    # load sample names
    sample_names = pd.read_csv(sys.argv[3], header=None)[0]
    ...
    for sample_id in sample_seg_map.keys():
        ...
        print(sample_name)
        with open(os.path.join('./data/GherigGraphQuality/oldsubgraph', f'{coasm_sz}_sample_ssasm_sscpg_sslr_{sample_name}.gfa'), 'w') as f:
