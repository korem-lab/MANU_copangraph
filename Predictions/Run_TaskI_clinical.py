...
def main(input_dir, task_name, robust, output_dir=None, specific_metadata=None, task_code=None, dry_run=False):
    ...
    model_params = {
        'num_leaves': [-1, 10, 20, 30],  # Maximum number of leaves in one tree
        'max_depth': [-1, 5, 10, 15],  # Maximum depth of each tree
        'learning_rate': [0.001, 0.01],  # Step size shrinkage
        'n_estimators': [1000],  # Number of boosting rounds (trees)
        'subsample': [0.7, 0.8, 0.9, 1.0],  # Fraction of samples used for training
        'colsample_bytree': [0.7, 0.8, 0.9, 1.0],  # Fraction of features used for training in each tree
        'reg_alpha': [0.0, 0.1, 0.5],  # L1 regularization term on weights
        'reg_lambda': [0.0, 0.1, 0.5],  # L2 regularization term on weights
        'min_child_samples': [1, 5, 10, 20]  # Minimum number of data needed in a leaf
    }
