...
def get_X(labels, organism_specific):
    ...
    humann_df_loc = '/manitou/pmg/projects/korem_lab/Projects/MOMSPI_COPAN_PREDICTION/HMN/humann_MOMS_PI_genefamilies_filtered_MLready.csv'
    
    print(f"Reading humann table from {humann_df_loc}")
    X = pd.read_csv(humann_df_loc, index_col=0).fillna(0)
...
def main(output_dir, task_name, labels, robust, run_params, model_params, dry_run, iterations=250, ofld=10, ifld=5):
    ...
    pred.run(
        ...
        hours = 12,
        cpus=2,
        out_file=f'{output_dir}/humann.orgspc={ORGANISM_SPECIFIC}.{task_name}.prediction.pkl',
        save_features=False,
...
if __name__ == '__main__':
    ...
    if DEBUG:
        print("DEBUG")
        main(
            output_dir='../data/Predictions/moms-pi/predictions', 
            task_name='momspi_hmn_p3', 
            labels='../data/Predictions/moms-pi/random.outcome.csv', 
            robust=3, 
            run_params='parameters/TaskI_run_params_3.yaml', 
            model_params='parameters/TaskI_model_params_3.yaml',
            iterations=100,
            dry_run=False, 
        )
    else:
