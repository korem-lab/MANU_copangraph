...
def get_X(graph_prefix, out_prefix, nodes, edges, abund=False):
    ...
    if abund and not edges:
        print("ABUNDANCE")
        nodes = pd.read_pickle(f'{graph_prefix}.abnd_mat.pkl').T.astype(float)
        ...
        nodes_features.to_csv(f'{out_prefix}.abnd.feature_map.csv', index=None, header=None)
        ...
        print("NODES+EDGES")
        nodes = pd.read_csv(f'{graph_prefix}.ncolor.csv', index_col=0).T.astype(np.uint8)
        edges = pd.read_csv(f'{graph_prefix}.ecolor.csv', index_col=0).T.astype(np.uint8)
        nodes_features = nodes.columns.to_series()
        edges_features = edges.columns.to_series()
        nodes_features.index = range(len(nodes_features))
        edges_features.index = range(len(edges_features))
...
def main(input_dir, graph_name, output_dir, task_name, labels, robust, run_params, model_params, dry_run, nodes, edges, iterations=250, ofld=10, ifld=5, corr=1):
    ...
    X = get_X(os.path.join(input_dir, graph_name), os.path.join(output_dir, task_name), nodes, edges)
